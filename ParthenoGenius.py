...
if args.het_scan == "1":
    ...
    with open(outfile_locus_scan, 'w') as fileobject: #with homozygosity summary outfile open for writing
        ...
        fileobject.write(f"\tMaximum number of homozygous loci expected to differ between mom and offspring based on error rate: {round((len(mom_homozyg))*float(args.error))}\n")
        ...
        fileobject.write(f"\tMaximum number of offspring loci expected to exhibit alternate alleles from maternal genotype based on error rate: {round((len(mom_heterozyg))*float(args.error))}\n")
        ...
        fileobject.write(f"\tMaximum number of loci expected to differ between mom and offspring based on error rate: {round((column_total)*float(args.error))}\n")
    ...
    with open(outfile_locus_scan, 'w') as fileobject: #with homozygosity summary outfile open for writing
        ...
        if ((len(males_homo)) <= (float(args.error)*len(mom_homozyg))):
            fileobject.write(f"THE OFFSPRING IS LIKELY A PARTHENOGEN.\n")
            fileobject.write(f"\n\tThe number of mom's loci for which at least one of offspring's alleles differs from maternal alleles is less than expected from the genotyping error rate alone.\n")
            fileobject.write(f"\tIncongruence between maternal genotypes and offspring genotypes is likely due to genotyping error rather than the presence of paternal alleles.")
        else:
            fileobject.write(f"THE OFFSPRING IS UNLIKELY TO BE A PARTHENOGEN.\n")
            fileobject.write(f"\n\tThe number of mom's loci for which at least one of offspring's alleles differs from maternal alleles is greater than the number expected from the genotyping error rate alone.\n")
            fileobject.write(f"\tIncongruence between maternal genotypes and offspring genotypes may be due to presence of paternal alleles.\n\n\n")
            fileobject.write(f"Note: Maternal heterozygosity analysis will not be conducted to test for mode of parthenogenesis.\n")
            fileobject.write(f"Note: Heterozygosity outfiles will not be created.")
    ...
    if ((len(males_homo)+len(males_het)) <= (float(args.error)*column_total)): #if evidence of parthenogenesis
        ...
        with open(outfile_heterozyg_sum_part2, 'w') as fileobject: #with the heterozygosity summary outfile open for writing
            ...
            if ((len(males)/len(mom_het))) > float(estim_error):
                ...
                fileobject.write(f"\tThe proportion of mom's heterozygous loci for which offspring has retained heterozygosity is greater than the estimated error rate from the maternal locus scan.\n")
                ...
                fileobject.write(f"\tThe proportion of mom's heterozygous loci for which offspring has retained heterozygosity is less than the estimated error rate from the maternal locus scan.\n")
